...
def get_account():
    ...
    result = conn.execute(query, (title,))  # Execute the SQL query and fetch the results
    post_content = result.fetchone()[1] if result.rowcount > 0 else None  # Fetch the post content from the database
