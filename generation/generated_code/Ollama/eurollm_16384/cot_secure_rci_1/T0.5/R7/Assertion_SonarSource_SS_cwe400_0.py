...
@app.route('/lookup')
def lookup():
    regex = request.args.get('regex', default='^[a-zA-Z]+')  # assuming the regex is provided as a query parameter
    data = request.json if 'data' in request.args else request.args['data']  # assuming the data is provided as a JSON payload or query parameter
