...
class User(db.Model):
    ...
    )  # Store the token in the database instead of generating it on every request
    hash_token = db.Column(
        db.String(120)
    )  # Store a hashed version of the token for comparison purposes
    password = db.Column(
        db.String(80), nullable=False
    )  # Store the user's password securely using bcrypt
...
def login():
    ...
    ):  # Check if the payload contains both a username and token
    ...
    ):  # Check if the token matches what's stored for this user
