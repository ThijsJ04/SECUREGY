...
def upload_img():
    ...
    if not os.path.exists("./assets/img"):
        os.makedirs(
            "./assets/img"
        )  # Create the assets/img directory if it doesn't exist

    decoded_data = base64.b64decode(data)  # Decode the image data from base64
    filepath = (
        f"../assets/img/{filename}.png"  # Create the file path for saving the image
    )
    ...
    )  # Return a success response with the image filename
