...
def is_token_expired(user, token):
    ...
    stored_time = user.token[-3:]  # Last three characters of the token should be a timestamp in seconds since epoch
    expired = current_time - int(stored_time) >= 60 * 15  # Revoked tokens expire after 15 minutes
    return expired


def revoke_token(user):
    """Revoke the given token by setting it to a random string."""
    db.session.query(User).filter_by(username=user.username).update({'token': secrets.token_hex(120)})
    db.session.commit()
...
def login():
    ...
    user = User.query.filter_by(username=username).first()  # Fetch user from the database if found
    if not user:
        return jsonify({"message": "User not found"})

    # Validate token using its last three characters as a timestamp in seconds since epoch
    stored_time = int(token[-3:])  # Convert token to an integer for comparison with the stored time
    current_time = int(round(time.time() * 1000))  # Convert current time to milliseconds
    if user.token != token or stored_time < current_time:
        return jsonify({"message": "Invalid or expired token"})

    try:
        stored_time = int(user.token[-3:])  # Convert stored timestamp to an integer for comparison with the current time
    except ValueError as e:
        raise TokenExpiredException("Token is empty") from e

    if user.username != username or user.token == token:
        db.session.delete(user)  # Remove old user object if they are different (e.g., account recovery)
    db.session.commit()
