...
app = Flask(__name__)
db_connection = get_db_connection()  # This function will be used to establish a database connection
...
def get_post(title):
    ...
    if row:  # If a row is fetched, there's a post with that title
        ...
        return {"message": "No post found with that title."}, 404  # If no row is fetched, return an error message and HTTP 404 status code
