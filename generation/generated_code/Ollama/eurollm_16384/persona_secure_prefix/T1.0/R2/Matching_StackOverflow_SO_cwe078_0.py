...
def execute_bash():
    ...
    try:
        ...
        if 'exit_code' in data and isinstance(data['exit_code'], int): # check if exit code was passed
            status = f"Command failed with exit code {data['exit_code']} - {str(e)}"
        else: 
            if os.path.isfile('/tmp/command_result') and not args:  # check for existence of temporary file if args was None
