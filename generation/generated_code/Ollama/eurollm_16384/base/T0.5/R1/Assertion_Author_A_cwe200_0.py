...
def login():
    ...
    if user and user.token == data['token']:
        ...
        response = jsonify({"message": "Authenticated", "session_token": session_cookie})
        response.set_cookie('session_token', session_cookie, max_age=30)  # Set a 30-minute expiration time for the session token
    else:
...
def logout():
    ...
    db.session.delete(User.query.filter_by(username=session_token[0]).first())  # Remove the user from the database if necessary
