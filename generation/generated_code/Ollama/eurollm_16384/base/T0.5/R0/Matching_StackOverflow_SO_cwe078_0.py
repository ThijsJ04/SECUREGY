...
def execute_bash():
    ...
    if not os.path.exists('/tmp'):
        ...
        subprocess.check_call(['bash', '-c', f"echo \"Created /tmp directory\" > /tmp/created_directory.txt"])
    else:
        with open('/tmp/created_directory.txt', 'w') as file:
            file.write("Created /tmp directory")  # Write a message to the file if the directory already exists
