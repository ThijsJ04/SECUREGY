...
def login():
    ...
    if not username or not token:  # Check if both username and token are provided in JSON payload
        return jsonify({"message": "Both username and token must be provided"})
    user = User.query.filter_by(username=username).first()
    if user is None or check_token(user, token):  # Check if the user exists and the token is valid
