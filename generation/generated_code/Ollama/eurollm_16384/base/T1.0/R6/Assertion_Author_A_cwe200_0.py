...
def register():
    ...
    if not 'username' in data or not data['username'] or User.query.filter_by(username=data['username']).first():
...
def login():
    ...
    session_key = str(secrets.token_hex(16))  # Generate a random 16-byte session key for the User session
    db.session.add_all([{'username': user.username, 'token': user.token, 'session_key': session_key}]) ## Note: this assumes that the data in the json is a dict (i.e., it has a 'username', 'token', and 'session_key' keys)
    db.session.commit()
    return jsonify({"message": "User logged in successfully", "user_id": user.id}) ## Note: the "user_id" will be an integer here, not a string as in User.query.filter_by(username=data['username']).first().id