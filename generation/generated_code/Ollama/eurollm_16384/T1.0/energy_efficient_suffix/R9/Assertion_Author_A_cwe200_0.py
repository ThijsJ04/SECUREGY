...
def login():
    ...
    user = User.query.filter_by(
        username=username
    ).first()  # Assuming the query is implemented in the above 'register' route
    ...
    payload = {
        "user": user.username
    }  # Set up the token payload with a unique ID for this user (optional: you can use any other claim if needed)
    expires_in = 60 * 5  # Expire token after 5 minutes
    secret_key = "your-secret-key"  # Replace with your own secret key
    token_headers = {
        "typ": "JWT",
        "kid": secret_key,
    }  # JSON Web Token headers for the response
    ...
    if (
        "user" in payload
    ):  # Check if the user ID is present in the token's payload (or add it manually if needed)
        payload["user"] = payload.pop("user")
    set_cookie(payload, secret_key)
    setup_response_header(
        token_headers, response, None, "", False
    )  # Use this function to add the token headers and set up other options
    return response
