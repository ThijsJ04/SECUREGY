...
def login():
    ...
    token = data.get(
        "token", ""
    )  # Token is optional but must be provided to make the method safe from CSRF attacks
    user = User.query.filter_by(username=username).first()
    if user and (
        user.token == token or (not token)
    ):  # Check if the provided token matches any stored tokens for the given username
        return jsonify(
