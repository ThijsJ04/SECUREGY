...
def unmarshal_data():
    ...
    if "serialized_data" in data:
        try:
            ...
            return jsonify(
                {"unmarshalled_data": unmarshalled_data.__dict__}
            )  # Use `__dict__` to preserve Python object attributes during unmarshalling
        except:
    ...
    return jsonify(
        data
    ), 400  # Return an error message instead of exiting with a generic exception
