...
app.config["MAX_CONTENT_LENGTH"] = (
    16 * 1024 * 1024
)  # Limit content length for file uploads
    ...
    if "filename" in request.files:  # Check if a file was uploaded
        ...
        if file and allowed_file(secure_filename):
            ...
            result = subprocess.check_output(
                command, shell=True
            )  # Execute the Bash command with 'shell=True' for shebang support
            return Response(
                content=result.decode("utf-8"), status=0, mimetype="text/plain"
            )
        else:
            return "Invalid file", 415
    elif (
        "command" in request.form
    ):  # If no file was uploaded, assume command is in POST data
        print("Executing command")
        result = subprocess.check_output(
            command + " &> /dev/null", shell=True
        )  # Don't use shell=True for your final executable version
