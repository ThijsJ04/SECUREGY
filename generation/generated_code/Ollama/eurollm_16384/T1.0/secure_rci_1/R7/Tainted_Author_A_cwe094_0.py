...
def contents_dict():
    ...
    try:
        result_dict = eval(
            content, {}, {}
        )  # Evaluate the JSON data as a Python dictionary
    except Exception as ex:
        print(ex)
        return jsonify({"error": "Invalid JSON content"}), 400
    try:
        result = (
            result_dict["result"] if "result" in result_dict else ""
        )  # Get the specific key or return an empty string if missing
    except Exception as ex:
        print(ex)
    ...
    try:
        output = eval(
            result, {}, {}
        )  # Evaluate the returned value (which should be a string or dictionary now) as a Python dictionary
    except Exception as ex:
        print(ex)
    ...
    try:
        final_output = (
            output["final_output"] if "final_output" in output else ""
        )  # Get the specific key or return an empty string if missing
    except Exception as ex:
        print(ex)
        return jsonify({"error": "Invalid dictionary structure"}), 400
