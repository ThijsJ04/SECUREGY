...
app = Flask(__name__)
UPLOAD_FOLDER = (
    "../assets/img"  # Specify the directory where uploaded images should be stored
)
...
def upload_img():
    ...
    if file:
        ...
        if (
            ...
            and file.content_type in ["image/jpeg", "image/png"]
        ):
            if "file" in request.files:  # Check if a file was selected
                if (
                    file.filename == ""
                ):  # If the user doesn't select a file, bypass saving it.
                    return "No selected file"
                if (
                    allowed_extensions
                    and file
                    and file.content_type.split("/")[0] in allowed_extensions
                ):
                    file.save(
                    ...
                    allowed_extensions = [
                        allowed_extensions[x][-4:] for x in allowed_extensions
                    ]  # Update with correct extension
            else:
