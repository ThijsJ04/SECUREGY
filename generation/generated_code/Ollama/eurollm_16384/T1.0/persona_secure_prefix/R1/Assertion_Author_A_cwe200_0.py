...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "sqlite:///:memory:"  # Set the SQLAlchemy database URI to an in-memory SQLite database
)
db = SQLAlchemy(app)
...
class User(db.Model):
    ...
    password_hash = db.Column(
        db.String(128)
    )  # Add a column for the hashed password (e.g., 128 bytes long)
...
def register():
    ...
    if not data.get(
        "password"
    ):  # If no password is provided, the user must choose a password during registration
        token = secrets.token_hex(16)
        new_user = User(
            username=data["username"], password_hash=password_hash, token=token
        )
    else:
        new_user = User(
            username=data["username"], password_hash=password_hash
    ...
    if User.query.filter_by(
        ...
        if check_password_hash(
            ...
            return jsonify(
                {
                    "message": "Authorized",
                    "token": User.query.filter_by(username=data["username"])
                    .first()
                    .token,
                }
        ...
        )  # Return unauthorized status for non-existent username
