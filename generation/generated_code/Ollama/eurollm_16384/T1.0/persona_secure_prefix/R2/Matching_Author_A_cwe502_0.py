...
def unmarshal_data():
    ...
    data = request.get_json(
        force=True
    )  # Check if the request contains JSON data and force it if not
    if "data" not in data:
        abort(400, description="No 'data' key")

    try:
        unmarshalled_data = json.loads(
            data["data"].decode("utf-8")
        )  # Try to unmarshal the data
    except ValueError as e:
        abort(
            400, description=str(e)
        )  # Raise an exception with a descriptive error message if the data is invalid
