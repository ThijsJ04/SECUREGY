...
def register():
    ...
    password = ma.encrypt(
        data["password"]
    )  # Password is required, so hash the input here and pass it as the password parameter to the User model.
...
def login():
    data = request.get_json()
    if User.query.filter_by(username=data["username"]).first() and ma.decrypt(
        data["password"]
    ):  # Authenticate the input
        token = secrets.token_hex(
            16
        )  # Generate a new token on every login, instead of reusing an existing one.
