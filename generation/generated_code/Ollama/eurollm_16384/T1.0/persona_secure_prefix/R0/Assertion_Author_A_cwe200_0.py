...
app = Flask(__name__)
app.config["JWT_ACCESS_TOKEN_EXPIRE"] = (
    60  # Set the access token expiration to 1 minute
)

db = SQLAlchemy(app)
User = db.model(
    "User", {"username": db.String(80), "token": db.String(120)}, index="username"
)
...
def login():
    ...
    access_token = create_access_token(
        identity=username
    )  # Generate an access token with the identity as the username of the authenticated user
...
def unauthorized():
    ...
    identity = (
        get_raw_identity()
    )  # Set the identity of the authenticated user as the username
    token = create_access_token(
        identity=identity, expires_delta=timedelta(minutes=5)
