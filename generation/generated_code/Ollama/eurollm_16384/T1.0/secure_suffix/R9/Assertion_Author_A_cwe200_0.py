...
def login():
    ...
    current_user = User.query.filter_by(
        username=username
    ).first()  # Fetch the user from the database by username
    if not current_user or check_password_hash(
        current_user.password_hash, generate_password_hash(token)
    ):  # Check if password hash matches
        return jsonify({"message": "Invalid username and/or token"})

    db.session.refresh(
        current_user
    )  # Refresh the user object for its updated `username` column
    if (
    ...
    db.session.add(
        current_user
    )  # Add the user back to the database since we've confirmed it exists
    db.session.commit()
