...
class User(db.Model):
    ...
    )  # Store the token for authentication purposes
...
def register():
    ...
    hashed_token = secrets.hmac(
        "sha512", f"{data['password']}".encode(), digestmod=hashlib.sha512
    ).hexdigest()  # Use a secure HMAC hashing algorithm
    token = db.Column(
        db.String(len(hashed_token), encode=False)
    )  # Store the token in the database for authentication purposes
...
def login():
    ...
    try:
        token_data = secrets.hmac(
            "sha512", f"{stored_token}{salt}".encode(), digestmod=hashlib.sha512
        ).hexdigest()  # Verify the token using HMAC hashing
        if (
            token_data == data["token"]
        ):  # Compare the hashed token in the database with the provided one
