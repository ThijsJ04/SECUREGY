...
def login():
    ...
    ).first()  # Search for a matching user by username
    if not existing_user or existing_user.token != data["token"]:
        return jsonify({"message": "Invalid credentials"}), 401
    db.session.refresh(
        existing_user
    )  # Refresh the User object in case it was migrated (e.g., on connection to a remote server)
