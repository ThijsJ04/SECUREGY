...
def eval_string():
    ...
    string = string[
        1:-1
    ]  # remove spaces on both sides of the first '{' or the last '}'

    # Evaluate it to a dictionary
    tree = ast.parse(string)
    dict_eval = eval(
        tree, {}, {}
    )  # eval() takes in a string (or AST), an empty environment, and a default value for new variables
    ...
    return str(
        dict_eval
    )  # return the dictionary as a string so it can be returned by Flask app routes
