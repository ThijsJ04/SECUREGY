...
def get_account():
    ...
    with get_db_connection() as conn:
        ...
        )  # Query the database with the provided title
        row = (
            cur.fetchone()
        )  # Retrieve the first matching row (assuming one record per post)
    ...
    content = (
        str(row[1]) + "\n" + str(row[2])
    )  # Assuming columns are 0 and 3, respectively
    return (
        "<p>Post Content:</p>" + content
    )  # Return the post content with proper HTML formatting
