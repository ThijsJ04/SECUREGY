...
def login():
    ...
    user = User.query.filter_by(username=username).first()  # Fetch user by username (not recommended in production)
    if not user or not validate_token(user, token):
        return jsonify({"message": "Invalid username or token"})
    return jsonify({"message": "Authorized", "username": username})  # Return the logged-in user's username for security reasons