...
def get_post():
    ...
    cur = conn.cursor()
    cur.execute(query, (title,))  # Execute a parameterized query to fetch the post's content
    row = cur.fetchone()  # Fetch the first row of the result set

    if row:  # If there is a match for the title
        return jsonify({"content": row[1], "title": row[2]})  # Return the post's content as JSON
    else:
