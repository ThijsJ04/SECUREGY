...
class User(db.Model):
    ...
    token = db.Column(db.String(120))  # Generated by the database itself, not by a secret function like secrets.token()
...
def register():
    ...
    token = User.generate_token()  # Generate a unique token using the database's token generation logic
...
def login():
    ...
    token = User.verify_token(data["token"])  # Verify the provided token against stored tokens using the database's logic
    if not token:
        return jsonify({"message": "Unauthorized"})
    return jsonify({"message": "Authorized", "token": None})  # No need to return the token here since it was verified earlier