...
def upload_img():
    try:
        ...
        file_data = request.files["img"]  # Assuming "img" is a key in the files dictionary

        # Encode image to base64 if it's not already encoded (for example, if image is uploaded as a file)
        data_encoded = None
        if isinstance(file_data, bytes):
            data_encoded = file_data
            base64.decode(file_data, "b")  # decode from base64 to bytes (used in this example for demonstration purposes)
    ...
    try:
        ...
        if data_encoded:  # Check if `data_encoded` is defined and not None (e.g., when saving a file)
            saved_image.write(base64.decode("b", data_encoded))
        else:
            with open('temp.png', 'wb') as tmp:
                tmp.write(file_data.read())  # Read the uploaded file content and save it to a temporary file
            saved_image.seek(0)
