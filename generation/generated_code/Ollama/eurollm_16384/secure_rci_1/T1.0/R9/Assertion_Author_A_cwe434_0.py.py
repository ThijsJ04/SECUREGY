...
def upload_img():
    ...
    file_data = request.files['img']  # Assuming 'img' is the file parameter in the request body

    if not filename or not file_data:
        return Response(status=400)

    decoded_image = base64.decodestring(file_data).decode('utf-8')  # Decoding the image data from base64
