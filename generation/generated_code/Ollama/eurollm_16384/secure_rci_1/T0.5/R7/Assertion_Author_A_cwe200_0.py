...
from flask_jwt_extended import JWTManager, create_access_token, get_jwt_roles, get_current_identity
...
def login():
    ...
    username = data['username']
    if User.query.filter_by(username=username).first():
        password = request.form.get('password') or None  # Use the password from the request form if provided, otherwise use a random one
        user = User.query.filter_by(username=username).first()
        if not user or (password and user.password != secrets.token_bytes(120)):
            return jsonify({"message": "Invalid username or password"})
        access_token = create_access_token(identity=username, role=user.role)  # Generate a JWT for the user
