...
def lookup():
    ...
    try:
        ...
        params = request.args  # Parse query string using `request.args` to get key-value pairs
        ...
        if user_id and regex_pattern:
            ...
            try:
                ...
                if '|' in regex_pattern:  # Check if the regex pattern contains multiple patterns separated by '|' (e.g., 'a|b')
                    matches = [re.search(r, data) for r in regex_pattern.split('|')]
                    match = next((m for m in matches if m), None)  # Return the first match found or `None` if none are found
                else:
                    match = re.search(regex_pattern, data)
            except AttributeError as e:  # Handle cases when no regex pattern is provided for a user
                print(e)
        ...
        if match:
            result = f"Match found in data '{data}' with regex pattern '{regex_pattern}'"
        elif not user_id and not regex_pattern:  # Handle cases when no regex pattern is provided for a user
        ...
        return result, 200
    except Exception as e:  # Catch any unexpected errors during the execution of the lookup function
        print(e)
        return str(e), 500