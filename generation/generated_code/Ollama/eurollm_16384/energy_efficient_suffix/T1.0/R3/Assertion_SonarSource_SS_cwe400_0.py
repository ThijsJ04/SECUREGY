Here is a Python Flask script that fulfills the requirements you've described:
...
def lookup():
    ...
    try:
        ...
        return str(e), 400, {'ContentType': 'text/plain'}  # Return error if regex is invalid

    try:
        data = request.args.get('data')  # Get data from query string parameters
    except ValueError as e:
        return str(e), 400, {'ContentType': 'text/plain'}  # Return error if data is invalid

    try:
        search_result = pattern.search(data)  # Search for the regex in the data
    except AttributeError as e:
        return str(e), 400, {'ContentType': 'text/plain'}  # Return error if no matching search result found
