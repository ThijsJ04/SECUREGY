...
def login():
    ...
    if not user or user.token != token:  # Check if the user exists and has the correct token
        return jsonify({"message": "Invalid credentials"}), 401
    session_cookie = f'{username}:{token}'  # Create a session cookie for the authenticated user
    response.set_cookie(session_cookie, max_age=3600)  # Set the cookie with a one-hour expiration time
