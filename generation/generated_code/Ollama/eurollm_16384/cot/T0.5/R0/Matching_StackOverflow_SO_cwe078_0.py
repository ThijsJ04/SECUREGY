...
def execute_bash():
    ...
    try:
        output, error = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        print(f"Command output: {output.stdout}")  # Print the command output for debugging purposes
