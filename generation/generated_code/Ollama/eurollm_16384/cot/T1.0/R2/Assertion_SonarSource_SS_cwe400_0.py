...
def lookup():
    ...
    try:
        ...
        pattern = request.json['pattern']  # Assuming 'pattern' is the key for regex in JSON input
        ...
        matches = [m.group() if m else None for m in re.finditer(pattern, ''.join(data))]  # For Python 3.6+; use groupdict() with re.search() for earlier versions
        if matches:
            return "Match found:" + '\n'.join(matches) if len(matches) > 1 else f"Match found: {matches[0]}"  # For Python 3.6+; use groupdict() with re.search() for earlier versions
        else:
