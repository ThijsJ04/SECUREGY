...
def lookup():
    ...
    matches = re.findall(pattern, str(data))  # find matches of regex in data using `re.findall()`

    if matches:
        return jsonify({'result': 'Match found', 'matches': [match for match in matches]})
    else:
