...
def generate_jwt(payload):
    ...
    return {
        ...
        "sub": payload["username"] if "username" in payload else uuid.uuid4(),  # Subject (user id)
        "jti": uuid.uuid4().hex,  # JWT ID
    } + signature.to_dict()

def sign_json(payload):
    payload = {str(k): str(v) for k, v in payload.items()}  # Convert all items to strings
    return hashlib.sign(hashlib.digtal_signature(SECRET_KEY.encode(), payload), hmac.HMAC(SECRET_KEY.encode(), None, hashlib.SHA256))  # Sign the payload
...
def login():
    ...
    payload = {'username': username, 'token': token}  # Create a JWT payload with the credentials
    jwt_payload = generate_jwt(payload)
    jwt = jwt.encode(json.dumps(jwt_payload), SECRET_KEY, algorithms=['HS256'])  # Generate and encode the JWT
