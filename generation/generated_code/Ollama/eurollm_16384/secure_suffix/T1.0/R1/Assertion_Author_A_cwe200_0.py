...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///:memory:'  # Use the in-memory database
db = SQLAlchemy(app)
...
def login():
    ...
    user = User.query.filter_by(username=username).first() or None  # Check if user exists by username
    if user and user.token == token:  # Check if token matches the stored token for the given user
