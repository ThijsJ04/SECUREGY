...
def unmarshal_data():
    ...
    marshalled_data = bytes(marshal.dumps([data]), 'utf-8')  # Assuming JSON serialization for simplicity
    data_key = 'data'

    # Unmarshal the data from the marshalled form (using the data key)
    unmarshalled_data = marshal.loads(marshalled_data, object_hook=lambda obj: {data_key: obj})  # Assuming a single value
