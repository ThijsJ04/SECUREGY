...
def register():
    ...
    password = data.get("password", "")  # Store the password as plaintext, not recommended in production
...
def login():
    ...
    if not user or check_password_hash(user.password_hash, token):  # Use the hashed password for comparison
