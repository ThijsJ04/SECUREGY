...
def upload_img():
    ...
    temp_dir = os.path.join(os.getcwd(), 'uploads', str(int(time.time() * 1000)))  # create a new temp dir each second
    if not os.path.exists(temp_dir):
        os.makedirs(temp_dir)
    path = os.path.join(temp_dir, filename + '.png')  # save the file with the same name as in the form

    # Save the base64 string to a file (use temp file to avoid problems when running script on production env)
    with open(path, "wb") as f:
        f.write(base64.decode(b64str))  # encode and write back into binary data using base64
