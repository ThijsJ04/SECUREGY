...
def login():
    ...
    session_token = secrets.token_hex(16)  # Generate a new token for the current session (optional)
    if not token or token != user.token:
        return jsonify({"message": "Invalid credentials"}), 401
    db.session.add(db.or_exc('existing', {'token': session_token}))  # Store the new session token
