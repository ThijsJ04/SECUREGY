...
def lookup():
    ...
    try:
        ...
        pattern = re.compile(request.args.get('pattern', '.*'))  # default pattern to '.*' (anything) for now
    ...
    try:
        ...
        data = [s.lower().split() for s in request.args.get('data', '')]  # default data to '' (empty) for now
    except ValueError as e:
